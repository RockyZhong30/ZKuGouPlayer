...
void ZShadowWidget::paintEvent(QPaintEvent *)
{
    //加阴影，因为扁平化去掉
//    QPainterPath path;
...
void ZShadowWidget::mousePressEvent(QMouseEvent *event)
{
    //只能是鼠标左键移动和改变大小
...
void ZShadowWidget::mouseReleaseEvent(QMouseEvent *)
{
    m_mousePress = false;
}

void ZShadowWidget::mouseMoveEvent(QMouseEvent *event)
{
    //移动窗口
    if(m_mousePress)
    {
